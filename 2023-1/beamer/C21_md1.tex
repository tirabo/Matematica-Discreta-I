...
\begin{teorema}\label{t5.5} Sea $T=(V,E)$  un grafo conexo. Entonces,  son equivalentes las siguientes propiedades
    \begin{enumerate}
...
{\color{blue} Esquema de la demostración} \pause
        \vskip .4cm

    (T1 $\Rightarrow$ T2) Si entre dos vértices hubiera dos caminos podríamos formar un ciclo: primero vamos de un vértice a otro por un camino y volvemos por el otro camino. \pause 
        \vskip .4cm
    (T2 $\Rightarrow$ T3) Se hará por inducción en $|V|$. \pause
    \vskip .2cm
    El caso base es $|V|=1$, en ese caso $|E|  =0 = |V| -1$. \pause
    \vskip .2cm
...
\begin{frame}
    ...
    El siguiente resultado se deduce de la demostración del teorema anterior. \pause
...
\begin{frame}
    ...
    También podemos representar esta función como un \textit{coloreo de vértices} que cumple que dos vértices adyacentes tienen distintos colores:
    \vskip .4cm\pause
\begin{center}
    \begin{tikzpicture}[scale=0.55]
        %\SetVertexSimple[Shape=circle,FillColor=white]
...
\begin{frame}
    ...
    Podemos representar el nuevo coloreo de vértices del  grafo de la siguiente manera:
