...
\begin{tikzpicture}[scale=0.8]
    ...
    \rta Denotamos $V$ la cantidad de vértices, $A$ la cantidad de aristas  y $C$ la cantidad de regiones. 

    (a) $V = 5$, $A=7$, $C = 4$, luego $C+V-A= 4 + 5 - 7 =2$.

    (b) $V = 5$, $A=7$, $C = 4$, luego $C+V-A= 4 + 5 - 7 =2$.

    (d) $V = 8$, $A=12$, $C = 6$, luego $C+V-A= 6 + 8  - 12 =2$.

    \item Demostrar que el grafo (d) del ejercicio \ref{ej-ady-a-grafos} es planar y comprobar la fórmula de Euler.
    
    \rta Recordemos que un grafo es planar si se \textit{puede dibujar} sin cruces de aristas. Claramente el grafo en cuestión se puede dibujar también de la siguiente forma

    \begin{tikzpicture}[scale=0.8]
        \Vertex[x=0+0.2,y=0-5.9]{1}
        \Vertex[x=2+0.2,y=0-5.9]{2}
        \Vertex[x=4+0.2,y=-3-5.9]{3}
        \Vertex[x=1.5+0.2,y=-3-5.9]{4}
        \Vertex[x=0+0.2,y=-3-5.9]{5}
        \Edges(1,2,3,4,5,1)
        \Edges(2,4,1)
        \Edge[style={bend left=90}](1)(3)
    \end{tikzpicture}

    \noindent que es un grafo planar. 
    
    En  este caso $V = 5$, $A=8$, $C = 5$, luego $C+V-A= 5 + 5  - 8 =2$.



    \item \label{ej-kn} ¿Cuántas aristas tiene el  grafo completo $K_n$? ¿Para qué valores de $n$ se puede afirmar que $K_n$ es planar?
    ...
    Para $n >5$ es claro que si $K_5$ no es planar, como $K_5$ es un subgrafo de $K_n$,  entonces $K_n$ no puede ser planar. 
    ...
    \vskip .3cm
    \begin{tabular}{ccc}
        \begin{tikzpicture}[every node/.style={draw,circle},scale=1]
        \SetVertexSimple[Shape=circle,FillColor=white,MinSize=8 pt]
