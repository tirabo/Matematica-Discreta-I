...
\begin{section}{Método binario para exponenciacion modular (*)}
...
\begin{proof}
    ...
    $$
\end{proof}
 La clave en la fórmula \eqref{eq-pmod-2} es que el exponente del término de la derecha es la mitad  que el del término de la izquierda. 
...
$$
donde $0 \le x, c, c' < 100000037$ y $s = 0, 1$. La clave es que cada uno de estos cálculos se realiza con facilidad. 

La fórmula de la proposición \ref{prop-exp-mod} nos muestra como fácilmente podemos obtener una fórmula recursiva para la exponenciación modular:  sea $n> 0$, definimos $\modulo$ el \textit{operador módulo} de tal forma que $a \modulo n$ devuelve el resto de dividir $a$ por $n$. También definimos a $\cociente$ el \textit{operador cociente} de tal forma que $ a \cociente n$ es el cociente de dividir $a$ por $n$. Entonces, por la proposición \ref{prop-exp-mod} se cumple que 
$$
a^d \modulo n \quad = \quad  a_0^{d_0}  a^r \modulo n,
$$
...
\end{align*}  
calcula $a^d \modulo n$. 

Esta función es fácilmente trasladable a pseudocódigo. 
...
En pseudocódigo la versión iterativa de la exponenciacion modular se puede describir de la siguiente forma. 
...
a0 = a
while cociente > 0:
    # invariante: a**d % n = em * a0**cociente * a0**resto 
    a0, em  = a0**2 % n, em * a**resto % n
    resto =  cociente % 2
    cociente = cociente // 2
em = em * a0**resto % n
        \end{verbatim}
...
\subsection*{Pruebas de primalidad}

En la implementación del  criptosistema RSA  es fundamental el uso de primos de grandes dimensiones, por ejemplo de más de $100$  dígitos. Ahora bien,  tratar de  demostrar que un número  (grande) es primo buscando sus divisores es imposible con una arquitectura de computadoras determinística,  como lo son las computadoras actuales. Justamente en eso se basa la fortaleza del criptosistema RSA. 

Sin embargo,  veremos en esta parte del apunte  que hay algoritmos que permiten determinar en forma probabilística si un número es primo o no. Mencionaremos al final de esta subsección algoritmos que determinan eficientemente y en forma determinística si un número es primo a o no. 


El primer  algoritmo que veremos es el test de primalidad Miller-Rabin probabilístico. Como su nombre lo indica  es una prueba probabilística de primalidad: un algoritmo que determina si un número dado es probable que sea primo.
...
\begin{lema}
    Sea $p$  un primo impar, entonces las únicas raíces cuadradas de $1$ modulo $p$ son $1$ y $-1$. Es decir, 
    $$
    x^2 \equiv 1 \pmod{p}  \quad \Rightarrow\quad x \equiv \pm 1 \pmod{p}.
    $$
\end{lema}
\begin{proof}
    Sea $x$ tal que $x^2 \equiv 1 \pmod{p}$, luego $x^2 - 1 \equiv 0 \pmod{p}$, como $x^2 -1 = (x-1)(x+1)$, obtenemos  $(x-1)(x+1) \equiv 0 \pmod{p}$. Esto quiere decir  que $p|  (x-1)(x+1)$. Como $p$  es primo, $p | x -1$ o $p| x +1$, es decir $x  \equiv 1 \pmod{p}$  o $x  \equiv -1 \pmod{p}$.
\end{proof}

\begin{teorema}
    Si $n$ es un primo impar, entonces $n$ es probable primo fuerte para cualquier base  $a$ ($0 < a < n$).
\end{teorema}
\begin{proof}
...
El test probabilístico de primalidad de Miller-Rabin se basa en las observaciones realizadas más arriba: sea $n$ entero positivo impar entonces, $n = 2s \cdot d + 1$ con $d$ impar, y sea $k$ entero positivo.  
\begin{enumerate}
...
Si $n$ es FPP las $k$ veces,  entonces decimos que $n$ supera el test probabilístico de primalidad de Miller-Rabin y tiene la probabilidad $1 - ( 1 / 4^k)$ de ser primo (y lo consideramos primo).  
    ...
    repetir k veces:
        ...
        if 1 == a**d % n:
            ...
            if fpp == False: # si no pasa la prueba
                return False
    return True # si pasó las k pruebas
            \end{verbatim}
        \end{small}
    \end{minipage}
