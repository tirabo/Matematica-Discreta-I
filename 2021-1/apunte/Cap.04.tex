...
\begin{section}{Método binario para exponenciacion modular (*)}\label{seccion-exponenciacion-modular}
    ...
    Una definición alternativa de las potencias de un número nos permite amuntar $d$  en forma significativa. Sea  $a$ número (entero, racional, real, etc.),  entonces
    \begin{equation}\label{def-potencia-2}
        a^d = \left\{
        \begin{matrix*}[l]
            1&,&\text{ si $d = 0$,} \\
            a \cdot a^{n-1}&,&\text{  si $d > 0$ y  $d$ impar;} \\
            {(a^2)}^{\frac{n}{2}}&,&\text{  si $d > 0$ y  $d$ par.}
        \end{matrix*}\right.   
    \end{equation}    
