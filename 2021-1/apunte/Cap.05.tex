...
que $T$ es conexo y no hay ciclos en $T$.
\end{definicion}
...
En cada componente conexa $T_i = (V_i, E_i)$ hay un único camino de un vértice a otro, pues sino esa propiedad no se cumpliría en $T$. Además, 
$$
...
Las propiedades \ref{T2} y \ref{T3} nos dan maneras alternativas de definir árboles. Por ejemplo la propiedad \ref{T2} puede ser considerada como la propiedad que define un árbol, en vez de  \ref{T1}. 
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Sean $v_{n-1},v_{n-2},\ldots,v_{n-r}$ los adyacentes a $v_n$. Como la valencia máxima es $k$, hay a lo más $k-1$ de ellos.
...
\end{equation*}
Hagamos la coloración considerando el orden $v_1, \ldots, v_n$. 
