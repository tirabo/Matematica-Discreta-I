...
Para explicar el algoritmo de Hierholzer nos es útil la siguiente definición:  un \textit{recorrido euleriano} es una caminata o circuito que no repite aristas. Un \textit{recorrido euleriano maximal}  es un recorrido euleriano que respetando la condición de no repetir aristas no es posible continuarlo.  
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Mientras en la caminata ya realizada exista un vértice $u$  que tenga aristas que no formen parte de la caminata, realice otro recorrido  euleriano maximal siguiendo las aristas no utilizadas. 
...
En  cualquier de los dos casos,  en el paso 1 el recorrido puede no cubrir todos los vértices y aristas del grafo inicial. Además,  observemos que el subgrafo que se obtiene eliminando las aristas del recorrido tiene todas las valencias pares. 
...
Primero daremos el pseudocódigo para un recorrido euleriano maximal.
\vskip .4cm    
\begin{minipage}{0.95\textwidth}
    \noindent \textsc{Recorrido euleriano maximal}
...
def rem(L, v_ini): 
    ...
    # post: devuelve recorrido euleriano que comienza en v_ini
    ...
    Ahora, implementamos el circuito euleriano o caminata euleriana,  según corresponda,  con el uso de la función \verb+rem()+.
...
cam = rem(libres, v_ini) # caminata
while len(libres.aristas()) > 0: 
    for v in libres.vertices(): 
        if len(libres.adyacentes(v)) > 0 and v in caminata:
            pos = cam.index(v) 
            cam  =  cam[:pos] + rem(libres, v) + cam[pos+1:]
\end{verbatim}
\end{small}
\end{minipage}
