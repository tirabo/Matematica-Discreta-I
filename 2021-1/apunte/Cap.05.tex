...
while libres.nro_aristas() > 0:
    ...
    pos = caminata.index(h) # en pos se insertará el rec. euleriano
    p0 = h
    sub_caminata = [p0]  # sub caminata
    while len(libres.adyacentes(p0)) > 0:
        p1 = libres.adyacentes(p0)[0]
        sub_caminata.append(p1)  # agrega p1 a caminata
        libres.quitar_arista([p0,p1]) # quitar arista p0, p1 
        p0 = p1 
    # sub_caminata es ahora un recorrido euleriano
    caminata =  caminata[:pos] + sub_caminata + caminata[pos+1:]
\end{verbatim}
\end{small}
\end{minipage}

En el algoritmo, \verb+libres.nro_aristas()+ indica el número de aristas en el grafo \verb+libres+, \verb+caminata.index(h)+ indica en que lugar de la caminata se encuentra \verb+h+ y \verb+libres.adyacentes(p0)+ indica la lista de vértices adyacentes a \verb+p0+ en el grafo \verb+libres+. 
