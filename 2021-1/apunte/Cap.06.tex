...
S = [0] # lista de vértices utilizados en el MST (comienza con 0)
Q = [1,...,n]  # lista de vértices aún no utilizados en el MST
L = [[1,0,p1],...,[n,0,pn]]]  # con pi = w(i,0)
# se irá modificando L de tal forma que si Q = [u0,...,ur], 
# L  = [[u0,v0,p0],...,[ur,vr,pr]] donde 
# vi = vértice en S adyacente a ui tal que w(ui,vi) es mínimo, 
# pi = w(ui,vi)
F = [[],...,[]] # grafo con vértices 0,...,n y sin aristas.
while Q != []:
    uk = vértice en Q tal que pk = w(uk,vk) es mínimo
    F[uk].append(vk)
    F[vk].append(uk) # agrega arista uk vk a F
    Q = Q - {uk}
    S.append(uk)
    L = L - {[uk,vk,pk]}
    for [ui,vi,pi] in L:
