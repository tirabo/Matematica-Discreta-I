...
\begin{frame}
    ...
    \begin{itemize}
        \item El orden natural de los enteros es tan importante como sus propiedades aritméticas. \vskip .4cm
        \item Expresamos la idea de orden formalmente diciendo que existe una relación que indicamos ``$<$''.\vskip .4cm
...
\begin{frame}
    ...
    \begin{beamercolorbox}[wd=\textwidth,rounded=true,shadow=true]{postit}%
        \begin{itemize}
            \item \textbf{($>$)} Diremos que $m>n$ si  $n<m$.\vskip .4cm
            \item \textbf{($\le$)} Diremos que $m \le n$ si $m<n$ o $m=n$. \vskip .4cm
...
\begin{frame}
    ...
    Es importante notar que el  axioma (\textbf{I11}) tiene una versión valedera para estos nuevos símbolos.\vskip 0.5cm
    \setbeamercolor{postit}{fg=black,bg=example text.fg!75!black!10!bg}
    \begin{beamercolorbox}[wd=\textwidth,rounded=true,shadow=true]{postit}%
    \begin{itemize}
    \item \textbf{($>$)} Si $a > b$ y $c>0$, entonces $ac > bc$.\vskip .4cm
    \item \textbf{($\le$)} Si $a \le b$ y $0 \le c$, entonces $ac \le bc$.\vskip .4cm
...
\begin{frame}
    ...
    El axioma (\textbf{I11}) tiene nuevas variante cuando consideramos la multiplicación de una desigualdád por enteros negativos.
...
\end{enumerate}
\vskip .8cm
El axioma (\textbf{I12}) es conocido como el     \textit{axioma de buena ordenación}  o  \textit{axioma del buen orden} o  \textit{principio de buena ordenación}.
\vskip .8cm
