...
# Un grafo se define con una lista de adyacencia G =[a_0,a_1,...]
#  1) los vertices son 0,1,...,len(G)-1
#  2) a_i lista de vertices adyacentes a i.
#  3) j in a_i sii i in a_j
#  4) a_i no tiene elementos repetidos (0 <= i <= len(G)-1) y está ordenado  creciente.
...
def aristas(graph):
    # pre: graph es un  grafo
    # post:  devuelve una lista de aristas, una copia de la lista de adyacencia
    return hgraph
