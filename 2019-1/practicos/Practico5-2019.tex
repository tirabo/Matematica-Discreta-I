...
\newcommand\Ppunto[2]{\psset{xunit=0.01cm} \psset{yunit=0.01cm}
\rput(#1,#2){$\scriptstyle\bullet$} \psset{xunit=0.1cm}
\psset{yunit=0.1cm}}
% coloca un punto grueso en (#1,#2), coordenadas en decimas de mm

% coloca frase #3 en 8pt en (#1,#2). Coordenadas en mm.
\newcommand\poner[3]{\rput(#1,#2){#3}}

% coloca frase #3 en 8pt en (#1,#2). Coordenadas en 1/10 mm.
\newcommand\pponer[3]{\psset{xunit=0.01cm} \psset{yunit=0.01cm} \rput(#1,#2){#3}
\psset{xunit=0.1cm} \psset{yunit=0.1cm}}

% coloca frase #3 en 10pt en (#1,#2). Coordenadas en mm.
\newcommand\ponerten[3]{\rput(#1,#2){#3}}

% coloca frase #3 en 10pt (#1,#2). Coordenadas en decima de mm.
\def\pponerten#1#2#3{\psset{xunit=0.01cm} \psset{yunit=0.01cm} \rput(#1,#2){#3}
\psset{xunit=0.1cm} \psset{yunit=0.1cm}}
...
% hace una linea de x,y a x',y' escala 1/10 mm
\newcommand\llinea[4]{\psset{xunit=0.01cm} \psset{yunit=0.01cm}
\psline(#1,#2)(#3,#4) \psset{xunit=0.1cm} \psset{yunit=0.1cm}}
...
\item Pruebe que si $G$ es un grafo con más de un vértice, entonces
existen dos vértices con la misma valencia.
...
\medskip
	...
	\item  Si $V=\{ v_1 \dots v_n \} $ y $\delta (v_i)=d_i \;\;\forall\,\;
	i=1,\dots , n\,$, calcule las valencias de el grafo complemento.
\end{enumerate}
