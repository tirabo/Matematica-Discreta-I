...
\begin{document}
	...
	\begin{ejercicio}
		...
		\noindent Un banco tiene que elegir un equipo directivo de $5$ personas entre un grupo de $8$ personas, de las cuales $3$ son hombres 
		\textbf{A}, \textbf{E}, \textbf{O} y $5$ mujeres \textbf{X}, \textbf{Y}, \textbf{Z}, \textbf{V}, \textbf{W}  
		¿De cuántas formas puede hacerse la elección si:
		
		\begin{enumerate}
			\item[(a)] no hay restricciones?
			
			\item[(b)] se eligen tres mujeres y dos hombres?
		\end{enumerate}
		
		\noindent Si ahora en el equipo directivo las personas deben cumplir roles distintos, es decir, se requiere elegir los $5$ cargos
		directivos: director(a), subdirector(a), interventor(a), cajero(a) y cobrador(a); 
		¿De cuántas formas puede hacerse la elección si:
		
		\begin{enumerate}
			
			\item[(c)] se eligen los tres hombres?
			
			\item[(d)] los hombres \textbf{A} y \textbf{E} no pueden estar juntos en la misma elección?
		\end{enumerate}
	...
	\begin{solucion}
		...
		\noindent (c) Los tres hombres ya están  elegidos (3 de 3) y hay $\binom{5}{2}$ posibles elecciones de mujeres. Con los 3 hombres y las 2 mujeres ya tenemos las 5 personas para el  equipo. Las formas de ordenar 5 personas son $5!$. Es decir el resultado total es
		\begin{equation*}
			\binom{5}{2} \cdot 5! =  \frac{5!}{3!2!} \cdot 5! = \frac{4 \cdot 5}{2}  \cdot 120 =10  \cdot 120 =\colorbox{lightgray}{1200}.
		\end{equation*}
		
		\begin{comment}
			
		
		Primero  elegimos los cargos a ser ocupados por los 3 hombres: para el primer hombre hay 5 posibilidades, para el segundo 4 y para el tercero 3. Es decir el total de posibles asignaciones de cargos a los 3 hombres es $5\cdot 4\cdot 3= 60$.
		
		
		Quedan dos cargos libres (los que dejaron libres los hombres). Para el  primer cargo hay 5 posibilidades (cualquiera de las 5 mujeres) y para el segundo quedan 4 elecciones. Luego el total de posibilidades para los dos cargos restantes es $5 \cdot 4 = 20$. 
		...
		\noindent (d)\textit{ Primera forma.} El  hecho de que \textbf{A} y \textbf{E} no están juntos en el equipo se puede partir en tres casos: 1) \textbf{A} y \textbf{E} no están en el equipo, 2) \textbf{A} está en el equipo y \textbf{E} no está en el equipo, y 3) \textbf{A} no está en el equipo y \textbf{E}  está en el equipo.
		...
		2) Ahora debemos pensar que \textbf{A} ocupa uno de los 5 cargos, por lo tanto hay 5 posibilidades (una por cada cargoo) y las  6  personas restantes (pues \textbf{E} no está) ocupan los 4 cargos restante: $6  \cdot 5 \cdot 4 \cdot 3 =360$ posibilidades. Por lo tanto el total de posibilidades es $5  \cdot  360 = 1800$.
